...
class ArucoLocalizerNode(Node):
    ...
    def image_cb(self, msg: Image):
        ...
        for i, marker_id in enumerate(ids.flatten()):
            ...
            # 마커 평면 좌표축을 SDF plane(normal=1 0 0, 즉 x축이 normal)과
            # 맞추기 위한 보정. solvePnP의 마커 z축(법선)을 SDF plane의 x축에 맞춰 회전.
            # marker local: x=오른쪽, y=위, z=마커가 보는 방향(법선)
            # 우리가 원하는 것: 마커 z축(법선) == world에서 해당 마커의 +x 또는 -x (yaw로 이미 표현됨)
            # -> 마커 local z축을 world x축으로 매핑하는 고정 회전
            R_marker_to_world_axis = tf_transformations.euler_matrix(0, math.pi/2, 0)[:3, :3]
            T_axis_fix = np.eye(4)
            T_axis_fix[:3, :3] = R_marker_to_world_axis

            T_optical_marker = T_optical_marker @ np.linalg.inv(T_axis_fix)
